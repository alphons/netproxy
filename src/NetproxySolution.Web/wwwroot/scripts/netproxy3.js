 ...
 *	@version    3.0.0 (last revision Mar, 2024)
...
(function()
{
    ...
    window.netproxy = function(url, data, onsuccess, onerror, onprogress)
    {
        ...
        // Automatische withCredentials-controle
        ...
        if (data instanceof FormData)
            xhr.timeout = 30000; // Alleen timeout voor FormData uploads
        ...
        xhr.onloadend = function()
        {
            ...
            if (xhr.status === 204)
            {
                if (typeof onsuccess === 'function')
                    onsuccess(null); // Geef expliciet `null` als resultaat
                return;
            }
            ...
            else if (xhr.status === 0)
            {
                if (xhr.timedout)
                {
                    const timeoutError = new Error(`Timeout van ${xhr.timeout}ms overschreden`);
                    if (typeof onerror === 'function')
                ...
                {
                    const networkError = new Error("Netwerkfout of verzoek geannuleerd");
                    if (typeof onerror === 'function')
        ...
        if (onprogress)
        {
            xhr.upload.onprogress = onprogress; // Voor uploads
            xhr.onprogress = onprogress;       // Voor downloads
        }
